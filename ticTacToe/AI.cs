...
namespace TicTacToe
{
    class AI
    {
        ...
        private bool computerMoveAt(int i, int j)
        {
        ...
        public bool playerMoveAt(int i, int j)
        {
            if (grid[i, j] == CellSelection.N)
            {
        ...
        private int minimax(CellSelection player)
        {
            if (didIWin(player))
            {
                return 1;
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    {
                        grid[i, j] = player;
                        int tempScore = -minimax(opponent(player));
                        if (tempScore > score)
                        {
                            score = tempScore;
                            bestRow = i;
        ...
        public Boolean didIWin(CellSelection marker)
        {
            int rowScore;
            int colScore;
